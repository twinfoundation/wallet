...
import type { IIotaStardustWalletConnectorConfig } from "./models/IIotaStardustWalletConnectorConfig";
import type { IIotaStardustWalletConnectorConstructorOptions } from "./models/IIotaStardustWalletConnectorConstructorOptions";

/**
 * Class for performing wallet operations on IOTA Stardust.
 */
export class IotaStardustWalletConnector implements IWalletConnector {
	/**
	 ...
	 */
	public readonly CLASS_NAME: string = nameof<IotaStardustWalletConnector>();

	/**
	 * The configuration to use for tangle operations.
	 * @internal
	 */
	private readonly _config: IIotaStardustWalletConnectorConfig;
	 ...
	 */
	constructor(options: IIotaStardustWalletConnectorConstructorOptions) {
		Guards.object(this.CLASS_NAME, nameof(options), options);
		Guards.object<IIotaStardustWalletConnectorConfig>(
			this.CLASS_NAME,
			nameof(options.config),
			options.config
		);
		Guards.object<IIotaStardustWalletConnectorConfig["clientOptions"]>(
			this.CLASS_NAME,
	...
	public async create(identity: string): Promise<void> {
		...
		await this._vaultConnector.setSecret<string>(
			IotaStardust.buildMnemonicKey(this._config, identity),
			mnemonic
		);
		const seed = Bip39.mnemonicToSeed(mnemonic);
		await this._vaultConnector.setSecret<string>(
			IotaStardust.buildSeedKey(this._config, identity),
	...
	public async getAddresses(
		...
		return IotaStardust.getAddresses(
			this._config,
